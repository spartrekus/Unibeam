    ...
    while( !feof(fp6) ) 
    {
	  ...
	  // fgets can be easily ported to things such as PHP !!
	    ...
	    /////////////////
            ...
            if ( fetchlinetmp[3] == ' ' )
            {
 	      fputs( strcut( fetchlinetmp, 3+2, strlen(fetchlinetmp)) , fp5 );
  	      fputs( "\\\\", fp5 );
  	      fputs( "\n", fp5 );
  	      foundcode = 1;
            } */
...
/* unibeam is too a little bit compatible with markdown, since it can produce for confort:
...
\begin{center}
\includegraphics[width=1.0\textwidth]{blabla/file.png}
\end{center}
*/
            /////////////////////////////////////////////////////////////////
            if ( foundcode == 0 )
	    ...
	    if ( strcmp( fetchcmdline, "!utf" ) == 0 )
            ...
            ////////////////////////////////////////////////
            // !braw, useful for reviewers (like !beginraw)
            if ( foundcode == 0 )
            if ( fetchline[0] == '!' ) 
            if ( fetchline[1] == 'b' )
            if ( fetchline[2] == 'r' )
	      ...
	      txtrawcode = 1;
            ...
            ////////////////////////////////////////////////
            // !eraw, useful for reviewers (like !beginraw)
            if ( foundcode == 0 )
            if ( fetchline[0] == '!' ) 
            if ( fetchline[1] == 'e' )
            if ( fetchline[2] == 'r' )
   ...
   if ( beamercode == 1 )
   {
     ...
     // end of noter
 	      ...
 	      fputs( strtrim( strcut( fetchline, 11+2, strlen(fetchline))) , fp5 );
            ...
            if ( foundcode == 0 )
            if ( fetchline[0] == '!' )
            if ( strcmp( strrlf( fetchline ) , "!language de" ) == 0 )
            {
                markup_language = 3;
  	        foundcode = 1;
            }
